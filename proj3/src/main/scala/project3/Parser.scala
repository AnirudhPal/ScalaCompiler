...
class Scanner(in: Reader[Char]) extends Reader[Tokens.Token] with Reporter {
  ...
  def getName() = {
    ...
    if (isKeyword(s)) Keyword(s)
		// Checks if name is Boolean and sets it as a Literal
    else if (isBoolean(s)) if (s == "true") Literal(true) else Literal(false)
    else Ident(s)
  }
...
class BaseParser(in: Scanner) extends Parser(in) {
  ...
  def parseOptionalType: Type = in.peek match {
		// Get Deliminator
    case Delim(':') =>
			// Consume that Deliminator
			in.next()
			// Call parseType on the following String
			parseType 
    case _ => UnknownType
  }
...
class SyntacticSugarParser(in: Scanner) extends BaseParser(in) {
  ...
  override def parseSimpleExpression = in.peek match {
		...
		case Keyword("if") =>
			val pos = in.next.pos
			accept('(')
			val bVal = parseSimpleExpression
			accept(')')
			val rB = parseSimpleExpression
			in.peek match {
				case Keyword("else") =>
					in.next()
					val lB = parseSimpleExpression
					If(bVal, rB, lB).withPos(pos)
				case _ =>
					val lB = Lit(()).withPos(pos)
					If(bVal, rB, lB).withPos(pos)
			}
...
class FunctionParser(in: Scanner) extends SyntacticSugarParser(in) {
  ...
  override def parseType = in.peek match {
		...
		case Delim('(') =>
			...
			in.next()
			// Get Types
			val tList = parseList[Type](parseType, ',', tok => tok match {
				case Ident(x) => true
				case _ => false
			})
			// Consume ')'
			...
			// Get other side
			val rt = parseType
			// Set Return Function Type
			FunType(tList.map(lt => ("", lt)), rt)
		// For <ident> | <type> '=>' <type>
    case _ => 
			val lt = super.parseType
			in.peek match {
				// For <type> '=>' <type>
				case Keyword("=>") =>
					// Consume '=>'
   ...
   */
  def parseArg: Arg = {
		// Get Argument name and position
		val (name, pos) = getName
		// Consume ':'
		accept(':')
		// Get Type
  ...
  def parseFunction: Exp = {
		...
		in.peek match {
			...
			case Delim(':') =>
				...
				accept(';')
				// Set Function
				FunDef(fname, args, ft, fbody).withPos(pos)
			// No Type Specified
			case _ =>
				// Consume '='
				accept('=')
				...
				accept(';')
				// Set Function
				FunDef(fname, args, UnknownType, fbody).withPos(pos)
		}  
}
...
class ArrayParser(in: Scanner) extends FunctionParser(in) {
  ...
  override def parseType = in.peek match {
    case Ident("Array") =>
			...
			// Consume '['
			accept('[')
			// Parse Type using previous class
			val at = super.parseType
			// Consume ']'
			accept(']')
			// Set Type
			ArrayType(at)
  ...
  override def parseTight = {
		...
		res match {
			...
			case App(fun, args) =>
          	...
          	val pos = fun.pos
          	accept('=')
          	val rhs = super.parseSimpleExpression
          	Prim("block-set", List(fun, args(0), rhs)).withPos(pos)
					case _ =>
						res
      	} 
			// Not Array
  ...
  override def parseSimpleExpression = in.peek match {
		...
		case Keyword("new") =>
			...
			accept('(')
			// Parse Array Size
			val as = super.parseSimpleExpression
			// Consume ')'
			accept(')')
			// Set Array
			ArrayDec(as, at)
