...
class SemanticAnalyzer(parser: Parser) extends Reporter with BugReporter {
  ...
  def typeBinOperator(op: String)(pos: Position) = op match {
    case "+" => FunType(List(("", IntType), ("", IntType)), IntType)
		// A[Int] (IOP) B[Int] = C[Int] 
		case "-" => FunType(List(("", IntType), ("", IntType)), IntType)
		case "*" => FunType(List(("", IntType), ("", IntType)), IntType)
		case "/" => FunType(List(("", IntType), ("", IntType)), IntType)
		// A[Int] (BOP) B[Int] = C[Bool]
  ...
  def typeTerOperator(op: String)(pos: Position) = op match {
		// A[ArrT](L[Int]) = V[Int] @133
		case "block-set" => FunType(List(("", ArrayType(IntType)), ("", IntType), ("", IntType)), UnitType)
    case _ =>
  ...
  def typeConforms(tp: Type, pt: Type)(env: TypeEnv, pos: Position): Type = (tp, pt) match {
    ...
    case (FunType(args1, rtp1), FunType(args2, rtp2)) if args1.length == args2.length => ??? // TODO: Function type conformity has been defined in Lecture 5.
			// Conform Arguments and Return Type
  ...
  def typeInfer(exp: Exp, pt: Type)(env: TypeEnv): Exp = exp match {
    ...
    case Prim(op, args) =>
      typeOperator(op, args.length)(exp.pos) match {
        case FunType(atps, rtp) => 
					...
					// IntOp and BoolOp
					else if(atps.length == 2) {
						Prim(op, List(typeCheck(args(0), IntType)(env), typeCheck(args(1), IntType)(env))).withType(rtp)
					} 
					// ArraySet
					else {
						Prim("block-set", List(typeCheck(args(0), ArrayType(IntType))(env), typeCheck(args(1), IntType)(env), typeCheck(args(2), IntType)(env))).withType(UnitType)
					}
    ...
    case Ref(x) =>
      env(x) match {
				// Ref
        case Some(tp) => Ref(x).withType(tp)
        case _ =>
    ...
    case If(cond, tBranch, eBranch) =>
      ...
      // then branch.
      // If
			val ebt = typeCheck(eBranch, pt)(env)
			If(typeCheck(cond, BooleanType)(env), typeCheck(tBranch, ebt.tp)(env), ebt).withType(ebt.tp)
    case VarDec(x, tp, rhs, body) =>
      if (env.isDefined(x))
        warn("reuse of variable name", exp.pos)
      // VarDec (Also Add to Env)
			val rht = typeCheck(rhs, tp)(env)
			val bt = typeCheck(body, pt)(env.withVar(x, rhs.tp)) 
			VarDec(x, rht.tp, rht, bt).withType(bt.tp)
    case VarAssign(x, rhs) =>
      ...
      pt match {
        ...
        case UnitType => 
					...
					val fake = Lit(())
					// Let
					Let(id, xtp, VarAssign(x, rht).withType(rht.tp), fake.withType(UnitType)).withType(UnitType)
				// Standard Assign
        case _ => VarAssign(x, rht).withType(rht.tp)
      }
    case While(cond, lbody, body) =>
			// Get Body Type
			val bt = typeCheck(body, pt)(env)
			// While
			While(typeCheck(cond, BooleanType)(env), typeCheck(lbody, UnitType)(env), bt).withType(bt.tp)
    case FunDef(fname, args, rtp, fbody) => 
    	// Unique Args
			checkDuplicateNames(args)
			// Get Body Type (Include Args in Env)
			var argenv = args.map(arg => (arg.name, arg.tp))
			//for (arg <- args) argenv.withVals(arg.name, arg.tp)
			val bt = typeCheck(fbody, rtp)(env.withVals(argenv))
			// FunDef
			FunDef(fname, args, bt.tp, bt).withType(funType(args, bt.tp))
